...
ELM_BENCHMARK_DIR=~/_REPOS_/elm-spa-example
...
do
    ...
    docker exec flamboyant_hypatia runhaskell /opt/elm/BuildFromSource.hs 0.18 &> .docker.compile.log ;
    echo "----------- copy backup" ;
    echo $i " --> " $COMPILE_OPTIONS
    cp -v $ELM_MAKE_BINARY ./elm-make.$COMPILE_OPTIONS ;
    echo "----------- cd $ELM_BENCHMARK_DIR"
    cd $ELM_BENCHMARK_DIR
    echo "----------- Clean artifacts" ;
    rm -rf $ELM_BENCHMARK_DIR/elm-stuff/build-artifacts ;
    echo "----------- perf with one core" ;
    perf stat sysconfcpus -n 1 $ELM_MAKE_BINARY $ELM_BENCHMARK_DIR/src/Main.elm &> make.$COMPILE_OPTIONS.onecore.perf ; 
    echo "----------- Clean artifacts again" ;
    rm -rf $ELM_BENCHMARK_DIR/elm-stuff/build-artifacts ;
    echo "----------- perf with all cores" ;
    perf stat $ELM_MAKE_BINARY $ELM_BENCHMARK_DIR/src/Main.elm &> make.$COMPILE_OPTIONS.multicore.perf ;
    echo "done"
done
